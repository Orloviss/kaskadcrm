...
router.post('/register', (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE username = ?', [username], (err, user) => {
    ...
    db.run('INSERT INTO users (username, password, role) VALUES (?, ?, ?)', [username, hash, role || 'admin'], function(err) {
      if (err) return res.status(500).json({ message: 'DB error' });
      const token = jwt.sign({ id: this.lastID, username, role: role || 'admin' }, JWT_SECRET, { expiresIn: '7d' });
      res.json({ token });
    });
  });
});
...
router.post('/change-password', authMiddleware, (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE id = ?', [req.user.id], (err, user) => {
    ...
    if (!bcrypt.compareSync(oldPassword, user.password)) return res.status(400).json({ message: 'Wrong old password' });
...
router.post('/change-login', authMiddleware, (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE username = ?', [newLogin], (err, user) => {
    ...
    db.run('UPDATE users SET username = ? WHERE id = ?', [newLogin, req.user.id], function(err) {
      if (err) return res.status(500).json({ message: 'DB error' });
      res.json({ success: true });
    });
  });
